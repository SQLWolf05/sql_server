...
---///	Client Counts FROM ACT
 ----------------------------------------------------------------------------------------------------
DECLARE @act_client_active AS INT		= 73304
DECLARE @act_client_inactive AS INT		= 0
DECLARE @act_patient_active AS INT		= 72498
DECLARE @act_patient_inactive AS INT	= 20244
DECLARE @act_deceased AS INT			= 0
...
---------These Queries Are To Be Used As Part Of The Trial Integrity Check For New Conversions-------
 ----------------------------------------------------------------------------------------------------
---///	Looks for patients without colors/patterns
 ----------------------------------------------------------------------------------------------------
if object_id('tempdb..#patient_color') is not null
 drop table #patient_color
 
create table #patient_color
(
 [id] INT IDENTITY (1,1)
, [Title] VARCHAR (40)
, [Tot Count]		INT
	 ...
	 ----------------------------------------------------------------------------------------------------
	---///	Load data into previous temp table #patient_color
	---///  These Queries Are To Be Used As Part Of The Trial Integrity Check For New Conversions
	---///	Looks for patients without colors/patterns
	 ----------------------------------------------------------------------------------------------------
	INSERT INTO #patient_color ([Title], [Tot Count]) 
	SELECT 'NoPatientColor' AS [Title], count(*) as tot FROM patient WHERE patientid not in (
	SELECT patientid FROM PatientAppearance)
	--Looks for patients without breeds
	UNION 
		SELECT 'NoPatientBreeds',count(*) FROM patient WHERE patientid not in(
		SELECT patientid FROM PatientBreeds)
		--Looks for patients without genders
		UNION
			SELECT 'NoPatientGender',count(*) as NoPatientGender FROM patient WHERE genderid is null
			--Looks for patients without species
			UNION
				SELECT 'NoPatientSpecies',count(*) as NoPatientSpecies FROM patient WHERE speciesid is null
				--Looks for weight records with no associated weight (i.e. weight = 0)
				UNION
					SELECT 'PetWeightIsZero', count(*) as PetWeightIsZero FROM PetWeight WHERE EnteredWeightinPounds <= 0

		SELECT * FROM #patient_color
...
'<html>
 ...
 <body>
 <h1>Note: This Server Time is operating on: ' + @server_time_zone + ' 
 <h1>ACT / VCA comparison' + '
 <table border = 1.25>
 <th> Title </th>
 <th> ACT Count </th>
 <th> VCA Count </th>
...
set @xml_breed_pct = @xml_breed_pct + @xml_actvcanumber + '</table>
 <br>
 <br>
 <br>
...
+ @xml_G2 + '</table>
 ...
 <br>
 <br>
<h1>Patients without colors / patterns</h1>
 <table border = .25>
 <tr>
 <th> Title </th>
 <th> Total Count </th>
